...
from python.search.depth_first_search.realization import Node, depth_first_search
...
def test_if_target_exists(tree: Node, target: int, output: Node):
    ...
    """
    assert depth_first_search(tree, target) == output
...
def test_if_target_does_not_exists(tree: Node, target: int):
    ...
    """
    assert depth_first_search(tree, target) is None
...
def test_for_empty_tree():
    ...
    """
    assert depth_first_search(Data.empty_tree, 1) is None
